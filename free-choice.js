...
(function(){
  ...
  function decorateChoice(){
    ...
    if(typeof val==='number')OPEN_CUSTOM.delete(stateKey);
    ...
    if(opened){
      ...
      editor.innerHTML=`<input type="text" maxlength="80" autocomplete="off" placeholder="写下你自己的答案……" value="${esc(customText(val))}"><small>写你真正想选的就好</small>`;
  ...
  function neutralResult(){
    ...
    if(hero){
      ...
      if(label)label.textContent=both===q.questions.length?'题都答好了':'题双方都有答案';
    }
  }

  // New history summaries are completion-only. Older stored summaries are also displayed neutrally.
  if(typeof roundsSummary==='function')roundsSummary=function(q,pairs){
    return {big:`${pairs.length} / ${q.questions.length}`,label:'题一起答完',chips:[],note:''};
  };
