...
function archive(q){
  ...
  if(currentVault)entry.cloudVaultHash=currentVault.vaultHash;
...
function cloudStatusHTML(entry){const s=cloudStatus(entry);return `<em class="cloud-status ${s.state}">${s.label}</em>`}
...
function bindCloudSync(button,entryId=null){if(!button)return;button.onclick=async()=>{button.disabled=true;const old=button.textContent;button.textContent='正在上传…';try{entryId?await syncOne(entryId):await syncLocal({associate:true});showToast('上传已处理');roundsHistoryList()}catch{showToast('暂时上传不了，本机记录还在')}finally{button.disabled=false;button.textContent=old}}}
...
async function rememberCurrentRoom(secret){if(!secret)return null;const vault=await deriveVault(secret);currentVault=vault;rememberVault(vault);return vault}
...
async function upsertEntry(entry,vault){
  ...
  const response=await fetchTimeout(`${SUPABASE_URL}/rest/v1/answer_history?on_conflict=vault_hash,entry_id`,{method:'POST',headers:{...requestHeaders(vault),Prefer:'resolution=merge-duplicates'},body:JSON.stringify(body)});if(!response.ok)throw new Error(`upload ${response.status}`);
}
...
async function syncOne(id){
  const list=roundsHistoryLoad(),entry=list.find(x=>x.id===id);if(!entry)throw new Error('entry missing');let vault=entry.cloudVaultHash?vaultByHash(entry.cloudVaultHash):currentVault;if(!vault&&window.coupleDuo?.roomSecret?.())vault=await rememberCurrentRoom(window.coupleDuo.roomSecret());if(!vault)throw new Error('no room vault');entry.cloudVaultHash=vault.vaultHash;entry.cloudError=false;roundsHistorySave(list,{sync:false});try{await upsertEntry(entry,vault);entry.cloudSyncedAt=Date.now();entry.cloudError=false;roundsHistorySave(list,{sync:false});return true}catch(error){entry.cloudError=true;roundsHistorySave(list,{sync:false});throw error}}
async function syncLocal({associate=false}={}){
  const list=roundsHistoryLoad();let attempted=0,succeeded=0,failed=0;
  for(const entry of list){if(isDeleted(entry.id))continue;let vault=entry.cloudVaultHash?vaultByHash(entry.cloudVaultHash):null;if(!vault&&associate)vault=currentVault;if(!vault)continue;if(!entry.cloudVaultHash)entry.cloudVaultHash=vault.vaultHash;attempted++;try{await upsertEntry(entry,vault);entry.cloudSyncedAt=Date.now();entry.cloudError=false;succeeded++}catch{entry.cloudError=true;failed++}}
  roundsHistorySave(list,{sync:false});await processDeleteQueue().catch(()=>{});if(failed)throw Object.assign(new Error('cloud sync failed'),{attempted,succeeded,failed});retryStep=0;return {attempted,succeeded,failed};
}
...
function scheduleSync(delay=500){clearTimeout(syncTimer);syncTimer=setTimeout(()=>syncLocal().catch(()=>{const d=retryDelays[Math.min(retryStep,retryDelays.length-1)];retryStep=Math.min(retryStep+1,retryDelays.length-1);clearTimeout(syncTimer);syncTimer=setTimeout(()=>syncLocal().catch(()=>{}),d)}),delay)}
...
window.coupleHistory={archive,open:roundsHistoryList,detail:roundsHistoryDetail,onRoomActivated,syncNow:()=>syncLocal({associate:true}),pullNow:pullCloud,statusFor,syncOne,currentVault:()=>currentVault};
window.coupleCloud={init:async()=>{if(window.coupleDuo?.roomSecret?.())await onRoomActivated(window.coupleDuo.roomSecret());else await pullCloud().catch(()=>{})},syncNow:()=>syncLocal({associate:true}),pullNow:pullCloud,deleteEntry:async(id,entry)=>{if(entry?.cloudVaultHash){saveDeleteQueue([...deleteQueue(),{entryId:id,vaultHash:entry.cloudVaultHash}]);await processDeleteQueue()}},statusFor};
