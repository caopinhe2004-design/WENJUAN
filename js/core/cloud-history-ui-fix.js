...
(function(){
  ...
  async function syncOne(id,button){
    ...
    try{
      const result=await c.syncEntry(id);
      await c.pullNow?.().catch(()=>{});
  ...
  function decorateList(){
    if(typeof route==='undefined'||route.view!=='history'||typeof app==='undefined'||!app)return;
  ...
  function decorateDetail(id=''){
    if(typeof route==='undefined'||route.view!=='history-detail'||typeof app==='undefined'||!app)return;
    if(id)detailId=id;
    ...
    button.textContent=s.state==='local'?'关联并上传':s.state==='synced'?'已上传':'立即上传';
    button.disabled=s.state==='synced';
  }
  ...
  if(typeof app!=='undefined'&&app&&typeof MutationObserver!=='undefined'){
